#!/usr/bin/env python3
"""
Visible Light Camera Capture for Pi Zero 2W
Captures visible spectrum images using Raspberry Pi Camera Module 3
Sends images to RPi5 for AI analysis via MQTT or network share

Deploy to: Cool Tower Visible, Warm Tower Visible
"""
...
def setup_mqtt():
    ...
    mqtt_client = mqtt.Client(client_id=f"{TOWER_NAME}_visible_cam")
...
def setup_camera():
    ...
    try:
        ...
        # Configure camera for visible light
...
def send_image_mqtt(filepath, file_size):
    """Send image metadata to RPi5 via MQTT"""
    try:
        metadata = {
            'tower': TOWER_NAME,
...
def main():
    """Main capture loop"""
    print(f"=== Visible Camera Starting ===")
    ...
    if not setup_camera():
        print("Warning: Camera setup failed. Running in mock mode.")
